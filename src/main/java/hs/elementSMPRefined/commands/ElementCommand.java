...
public class ElementCommand implements CommandExecutor, TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        return switch (args.length) {
            case 2 -> {
                ...
                if (subCmd.equals("bot")) {
                    yield filterStartingWith(List.of("spawn", "stop"), args[1]);
                }
            ...
            case 3 -> {
                String subCmd = args[0].toLowerCase();
                if (subCmd.equals("bot") && args[1].equalsIgnoreCase("spawn")) {
    ...
    private class BotCommand implements SubCommand {
        @Override
        public boolean execute(CommandSender sender, String[] args) {
            ...
            Optional<ElementType> element = parseElementType(args[2]);
            if (element.isEmpty()) {
                player.sendMessage(ChatColor.RED + "Unknown element. Choose: " + getElementNames().stream().collect(Collectors.joining(", ")));
                return true;
            }
