...
public class BotCommand implements ElementSubCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage(Lang.BOT_THIS_COMMAND_CAN_ONLY_BE);
            return true;
        }
        if (args.length < 2) {
            player.sendMessage(Lang.BOT_USAGE_ELEMENT_BOT_SPAWN_ELEMENT);
            return true;
        }
        if (args[1].equalsIgnoreCase("stop")) {
            plugin.getElementBotManager().stop(player);
            player.sendMessage(Lang.BOT_YOUR_ELEMENTAL_BOT_WAS_REMOVED);
            return true;
        }
        ...
        if (element.isEmpty()) {
            player.sendMessage(Lang.botUnknownElementChoose(String.join(", ", CommandSupport.getElementNames())));
            return true;
        }
